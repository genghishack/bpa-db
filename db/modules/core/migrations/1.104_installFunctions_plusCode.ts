const seq = 1.104;
const label = 'install pluscode functions';
...
export default {
  seq, label, sql: `
    ...
    CREATE OR REPLACE FUNCTION layer_mgt.pluscode_cliplatitude(
        lat numeric)
    RETURNS numeric
        LANGUAGE 'plpgsql'
        COST 100
    ...
    CREATE OR REPLACE FUNCTION layer_mgt.pluscode_normalizelongitude(
        lng numeric)
    RETURNS numeric
        LANGUAGE 'plpgsql'
        COST 100
    ...
    CREATE OR REPLACE FUNCTION layer_mgt.pluscode_isvalid(
        code text)
    RETURNS boolean
        LANGUAGE 'plpgsql'
        COST 100
    ...
    CREATE OR REPLACE FUNCTION layer_mgt.pluscode_codearea(
        latitudelo numeric,
        longitudelo numeric,
        latitudehi numeric,
        longitudehi numeric,
    ...
    CREATE OR REPLACE FUNCTION layer_mgt.pluscode_isshort(
        code text)
    RETURNS boolean
        LANGUAGE 'plpgsql'
        COST 100
    ...
    CREATE OR REPLACE FUNCTION layer_mgt.pluscode_isfull(
        code text)
    RETURNS boolean
        LANGUAGE 'plpgsql'
        COST 100
    ...
    CREATE OR REPLACE FUNCTION layer_mgt.pluscode_encode(
        _lat numeric,
        _lng numeric,
    ...
    CREATE OR REPLACE FUNCTION layer_mgt.pluscode_decode(
        code text)
    RETURNS TABLE(lat_lo numeric, lng_lo numeric, lat_hi numeric, lng_hi numeric, code_length numeric, lat_center numeric, lng_center numeric) 
        LANGUAGE 'plpgsql'
        COST 100
    ...
    CREATE OR REPLACE FUNCTION layer_mgt.pluscode_shorten(
        code text,
        latitude numeric,
        longitude numeric)
    RETURNS text
    ...
    CREATE OR REPLACE FUNCTION layer_mgt.pluscode_recovernearest(
        short_code text,
